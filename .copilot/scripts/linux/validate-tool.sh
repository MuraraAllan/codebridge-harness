...
if [[ "$tool_name" == "run_in_terminal" || "$tool_name" == "execute_command" || "$tool_name" == "runTerminalCommand" ]]; then
...
if [[ "$tool_name" == "edit_file" || "$tool_name" == "write_file" || "$tool_name" == "editFiles" || "$tool_name" == "createFile" ]]; then
