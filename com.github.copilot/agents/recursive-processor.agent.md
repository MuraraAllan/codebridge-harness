...
description: "Deflush user messages into task, intention, context, and document-read plans; recursively delegate comparable groups when needed, and delegate image/picture interpretation to contextImageInterpret."
user-invocable: false
tools: [read, search, web, agent]
agents: [recursive-processor, contextImageInterpret]
...
If the user message or input contains pictures, images, diagrams, or visual assets, immediately delegate the image interpretation task to `contextImageInterpret` before continuing deflushing.
